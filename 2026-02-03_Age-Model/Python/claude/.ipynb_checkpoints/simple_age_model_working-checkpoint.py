...
with pm.Model() as simple_model:
    ...
    if len(band_indices) > 0:
        ...
        band_months = (band_ages % 1.0) * 12
        
        # These should cluster around a particular month
        # Use von Mises (circular normal) distribution
        kappa = 5.0  # Concentration (higher = tighter clustering)
        
        # Convert months to radians
        band_months_rad = band_months * 2 * np.pi / 12
        band_month_rad = band_month * 2 * np.pi / 12
        
        # Likelihood: observed months should cluster around band_month
        pm.VonMises('band_timing',
                    mu=band_month_rad,
                    kappa=kappa,
                    observed=band_months_rad)
