...
# Reference epoch: midpoint of the SAG core averaging window.
# correction(t) = suess(T_REF) − suess(t)
#   → positive for samples younger than T_REF  (more depleted → add back)
#   → negative for samples older than T_REF    (pre-industrial → small downward shift)
# Applied only to d13c; d18o is unaffected by the Suess effect.

SUESS_T_REF = 1997.75   # decimal year; change if your reference epoch changes

_suess = pd.read_excel('C:\\Users\\mkara\\Desktop\\Academics\\Coral\\2026-04-01_Suess-Effect\\suess-effect.xlsx').sort_values('year')
_suess_fn = interp1d(
    _suess['year'].values,
    _suess['suess_d13c'].values,
    kind='linear',
    bounds_error=False,
    fill_value='extrapolate'
)
_suess_ref = float(_suess_fn(SUESS_T_REF))   # Suess value at reference epoch
#-----------------------------------------------------------------------------------------------
...
# Suess correction on raw irregular timestamps
#df_sorted['suess_correction'] = _suess_ref - _suess_fn(df_sorted['age_final'].values)
df_sorted['d13c_suess'] = df_sorted['d13c'] - _suess_fn(df_sorted['age_final'].values)

# MAs and std for d13c_suess (parallel to d13c)
df_sorted['d13c_suess_ma']  = df_sorted['d13c_suess'].rolling(window_size,  center=True).mean()
df_sorted['d13c_suess_ma2'] = df_sorted['d13c_suess'].rolling(window_size2, center=True).mean()
df_sorted['d13c_suess_ma3'] = df_sorted['d13c_suess'].rolling(window_size3, center=True).mean()
df_sorted['d13c_suess_std'] = df_sorted['d13c_suess'].rolling(window_size2, center=True).std()

# Tectonic-removed versions (MA2 and MA3 baselines, same as d13c)
df_sorted['d13c_suess_no_tectonic']  = df_sorted['d13c_suess'] - df_sorted['d13c_suess_ma2']
df_sorted['d13c_suess_no_tectonic2'] = df_sorted['d13c_suess'] - df_sorted['d13c_suess_ma3']
df_sorted['d13c_suess_notec_ma']     = df_sorted['d13c_suess_no_tectonic'].rolling(window_size, center=True).mean()

df_resampled_ = df_sorted[['age_final', 'age_dt', 'd13c', 'd13c_suess', 'd18o', 'd18o_pdb']].set_index('age_dt')
...
# Suess correction on resampled grid (re-evaluate interpolator at resampled ages)
df_resampled['suess_correction'] = _suess_ref - _suess_fn(df_resampled['age_resampled'].values)
df_resampled['d13c_suess'] = df_resampled['d13c'] + df_resampled['suess_correction']
...
# d13c_suess MAs and std on resampled grid
df_resampled['d13c_suess_ma']  = df_resampled['d13c_suess'].rolling(window_size,  center=True).mean()
df_resampled['d13c_suess_ma2'] = df_resampled['d13c_suess'].rolling(window_size2, center=True).mean()
df_resampled['d13c_suess_ma3'] = df_resampled['d13c_suess'].rolling(window_size3, center=True).mean()
df_resampled['d13c_suess_std'] = df_resampled['d13c_suess'].rolling(window_size2, center=True).std()
...
# Tectonic-removed versions for d13c_suess
df_resampled['d13c_suess_no_tectonic'] = df_resampled['d13c_suess'] - df_resampled['d13c_suess_ma2']
df_resampled['d13c_suess_notec_ma']    = df_resampled['d13c_suess_no_tectonic'].rolling(window_size, center=True).mean()
#-----------------------------------------------------------------------------------------------
...
#                              d13c_suess_sg_trend, d13c_suess_no_tec_sg
#                              d18o_sg_trend,       d18o_no_tec_sg
# New columns on df_resampled: same set

SG_WINDOW   = 73   # months (~6yr) on the monthly grid
SG_POLYORDER = 3

# Temporary monthly grid (used only to apply SG) — include d13c_suess
_df_monthly = (df_sorted[['age_final', 'age_dt', 'd13c', 'd13c_suess', 'd18o', 'd18o_pdb']]
               .set_index('age_dt')
               .resample('ME').mean()
               .interpolate())
_t_monthly = _df_monthly['age_final'].values

for col in ['d13c', 'd13c_suess', 'd18o', 'd18o_pdb']:
